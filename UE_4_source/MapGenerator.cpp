...
int MapGenerator::generatePaths()
{
    ...
    //making sure there aren't multiple starting or finishing positions
    int countS = 0;
    int countX = 0;

    for (int i = 0; i < MAP_Y; i++) {
        for (int j = 0; j < MAP_X; j++) {
            if (MAP_MATRIX[i][j] == 'S') countS++;
            else if (MAP_MATRIX[i][j] == 'X') countX++;
        }
    }

    if (countS > 1 || countX > 1) return 1;
